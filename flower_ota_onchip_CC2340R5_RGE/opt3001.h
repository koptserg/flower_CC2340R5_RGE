...
{
    SHUTDOWN = 0b00,    // Low power state (default)
...
typedef enum OPT3001_CONVERSION_TIME
{
    ...
    OPT3001_800MS = 1, // 800ms conversion time (default)
} OPT3001_conv_time_t;
...
typedef enum OPT3001_INTERRUPT_MODE
{
    ...
    OPT_INT_LATCHED = 1,    // User-cleared interrupts (default)
...
{
    OPT3001_RANGE_AUTO = 0b1100, // default
    OPT3001_RANGE_40 = 0,
    OPT3001_RANGE_80 = 1,
    OPT3001_RANGE_160 = 2,
    OPT3001_RANGE_320 = 3,
...
// public
    ...
    extern bool OPT3001_begin(uint8_t address);

    // Set the device i2c address of the sensor
    extern void OPT3001_set_address(uint8_t address);

    // Check that the controller is able to communicate with the sensor over i2c
    extern bool OPT3001_check_comms(void);

    // Apply the soft configuration to the sensor
    extern void OPT3001_apply_config(void);

    // Read the sensor's current configuration
    extern OPT3001_config_t OPT3001_read_config(void);

    // Get the illuminance measurement from the sensor as lux
    extern uint32_t OPT3001_get_illuminance(void);

    // Set the high limit for sensor measurements before faults occur
    extern void OPT3001_set_high_limit(OPT3001_result_t high_limit);

    // Get the sensor's current high limit level
    extern OPT3001_result_t OPT3001_get_high_limit(void);

    // Set the low limit for sensor measurements before faults occur
    extern void OPT3001_set_low_limit(OPT3001_result_t low_limit);

    // Get the sensor's current low limit level
    extern OPT3001_result_t get_low_limit();
